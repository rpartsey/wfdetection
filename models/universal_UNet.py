...
class GetEncoder:
    ...
    def get_encoder(self):
        if self.encoder_name == 'ResNet':
            ...
            raise NotImplementedError('Encoder {} is not implemented'.format(self.encoder_name))
    ...
    def _get_serresnext(self):
        ...
        encoder = se_resnext50_32x4d(num_classes=1000, pretrained='imagenet' if self.pretrained else None)
        bottom_channel_nr = 2048
...
class UNet(nn.Module):
    def __init__(self, encoder_name, device="cpu", encoder_depth=50, num_classes=2, pretrained=False, num_filters=32,
                 dropout_2d=0.2,
                 is_deconv=True, SCSE=False, freeze=False, **kwargs):
        super().__init__()
        print("Model pretrained:", pretrained, type(pretrained))
        self.dropout_2d = dropout_2d
        self.encoder = GetEncoder(encoder_name, encoder_depth=encoder_depth, pretrained=pretrained,
        ...
        for l in self.encoder:
            if l.startswith("conv") and freeze:
                for param in self.encoder[l].parameters():
                    param.requires_grad = False
            if l == 'bottom_channel_nr':
                continue
            self.encoder[l].to(device)
        for l in self.decoder:
            self.decoder[l].to(device)

        # TODO: remove this color or fix with something
        # self.color = nn.Conv2d(1, 3, kernel_size=(3, 3), padding=1)
        # n = self.color.kernel_size[0] * self.color.kernel_size[1] * self.color.out_channels
        # self.color.weight.data.normal_(0, math.sqrt(2. / n))
