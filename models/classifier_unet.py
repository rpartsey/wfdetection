...
from .classifier_r2_unet import Squeeze
...
class StackDecoder(nn.Module):
    ...
    def __init__(self, previous_x_channels, in_channels, out_channels, kernel_size=3,
                 uppsampling_method='conv_transpose'):
...
class UNet(nn.Module):
    ...
    def __init__(self, img_ch=3, output_ch=2, num_classes=2, **kwargs):
        super(UNet, self).__init__()

        # ENCODER PART                                          # Image Size:
        self.down_1 = StackEncoder(img_ch, 64, kernel_size=3)  # 1024
        ...
        self.classify = nn.Conv2d(32, output_ch, kernel_size=1, bias=True)
    ...
    def predict(self, x):
        self.eval()
        with torch.no_grad():
            out_segm, class_out = self.forward(x)
            class_out = class_out.argmax(1) == 0
            if out_segm.shape[1] == 2:
                out_segm = out_segm.softmax(1)
                out_segm[class_out] = 0.0
                out_segm = out_segm[:, 1:]
            else:
                out_segm = out_segm.sigmoid()
                out_segm[class_out] = 0.0
        return out_segm
